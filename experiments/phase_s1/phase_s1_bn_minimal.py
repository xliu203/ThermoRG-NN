#!/usr/bin/env python3
"""
ThermoRG Phase S1 v3 — BN Only (Minimal)
=========================================
只跑 BN D=48, 96 × 2 seeds，用于验证 φ_BN ≈ 0.66
Epochs 和当前 running 版本一致
"""
...
# ──────────────────────────────────────────────────────────────────────────────
# CONFIG — BN only, D=48,96
# ──────────────────────────────────────────────────────────────────────────────

CONFIGS = [
    ('BN_NoSkip',    'batchnorm',  [42, 123]),    # BatchNorm cooling
]

D_VALUES = [48, 96]           # 只跑关键点
EPOCHS = 300                  # 和当前 running 一致
BATCH_SIZE = 128
LR = 0.01
WD = 5e-4
MOMENTUM = 0.9
...
GAMMA_TRACK_EVERY = [50, 100, 150, 200, 250, 300]  # track gamma at these epochs
...
def get_dataloaders():
    ...
    transform_val = T.Compose([
        T.ToTensor(),
        T.Normalize([0.4914, 0.4822, 0.2466], [0.2470, 0.2435, 0.2616]),
    ])
...
def main():
            ...
            for seed in seeds:
                ...
                if run_name in completed:
                    if ckpt_path.exists():
                        ...
                        print(f"  [{config_name}] D={base_ch} seed={seed} — loaded from checkpoint")
                    continue
                ...
                start_epoch = 0
                resume_ckpt = CKPT_DIR / 'current_ckpt.pt'
                if resume_ckpt.exists():
                    ckpt = torch.load(resume_ckpt, map_location=DEVICE)
                    # Try to match by config
                    pass  # Per-run checkpoint instead
    ...
    # Load None baseline from v3 results if available
    v3_results_file = Path('./phase_s1_results_v3/phase_s1_results.json')
    base_beta = None
    if v3_results_file.exists():
        with open(v3_results_file) as f:
            v3 = json.load(f)
        none_cfg = next((c for c in v3['configs'] if c['name'] == 'None_NoSkip'), None)
        if none_cfg and none_cfg.get('scaling_fit', {}).get('beta'):
            base_beta = none_cfg['scaling_fit']['beta']
            print(f"Baseline (None_NoSkip) β = {base_beta:.4f}")

    print(f"\n{'Config':<15} {'β':<10} {'φ':<10} {'γ (D=48,96)':<15}")
    print("-" * 50)
    for cfg in all_results['configs']:
        ...
        print(f"{cfg['name']:<15} {beta:<10.4f} {phi:<10.3f} {avg_gamma:<15.4f}")

    if base_beta and all_results['configs']:
        bn_cfg = all_results['configs'][0]
        if bn_cfg.get('scaling_fit', {}).get('beta'):
            phi_bn = bn_cfg['scaling_fit']['beta'] / base_beta
            print(f"\n  φ_BN = {phi_bn:.3f}  (theory: ≈ 0.66)")
