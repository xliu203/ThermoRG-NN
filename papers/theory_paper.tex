...
\item \textbf{Limitations.} The physical laws provide an effective structural framework (form), but the flesh (constants $B$, $C$, $\gamma_c$) must be determined by thermodynamic contact with the specific environment (dataset). This sets up the companion Applied Paper on ThermoRG-AL.
\end{enumerate}
...
\end{equation}
Note: this empirically observed $D^{0.26}$ scaling should \emph{not} be interpreted as evidence for a GELU saturation regime. Gap 2 established that standard RMT at initialization predicts $\lambda_{\max} \propto D^{0.48}$, and Gap 2b showed that SGD training from standard initialization cannot reach the $D^{0.26}$ regime---within one training step, $\lambda_{\max}$ diverges sharply toward $+\infty$ (numerical overflow at $D \geq 256$), indicating a sharp spectral explosion rather than saturation. The physical mechanism behind the observed $\lambda_{\max} \propto D^{0.26}$ and the resulting $J_{\mathrm{topo}}(D) \propto D^{-\alpha_{\mathrm{GELU}}/L}$ remains an open question; standard RMT eigenvalue concentration combined with finite-width corrections is the more plausible working hypothesis.

\medskip\noindent\textbf{Step 4: Effective dimension scaling.} Gap 2 measures $D_{\mathrm{eff}}(D) \propto D^{0.93}$, a near-linear scaling that is not predicted by standard RMT. The analysis is complicated by the use of $\sigma=1.0$ weight initialization (not Kaiming normalization) in our experiments: under this initialization, $\|W\|_F^2 \propto D^2$ and standard RMT predicts $\lambda_{\max} \propto D$, giving $D_{\mathrm{eff}} \propto D^0$. The observed $D^{0.93}$ scaling arises from GELU saturation effects suppressing both $\lambda_{\max}$ (from $D^1$ to $D^{0.48}$) and $\|W\|_F^2$ (from $D^2$ to $D^{1.89}$) simultaneously. The precise exponents depend on the interaction between GELU nonlinearity and initialization scheme; a first-principles derivation remains an open problem. The empirically observed $D_{\mathrm{eff}} \propto D^{0.93}$ is used in subsequent analysis regardless of theoretical derivation. We note that this measurement used $\sigma=1.0$ initialization rather than Kaiming normalization; the dependence of $D_{\mathrm{eff}}$ scaling on initialization scheme remains an open question.
...
\end{equation}
\medskip\noindent\textbf{Open question:} Gap 2 establishes that at $\sigma=1.0$ initialization (not Kaiming), $\lambda_{\max} \propto D^{0.48}$ and $D_{\mathrm{eff}} \propto D^{0.93}$. The near-linear $D_{\mathrm{eff}}$ scaling arises from GELU saturation suppressing both $\lambda_{\max}$ (factor $D^{-0.52}$) and \$\|W\|_F^2$ (factor $D^{-0.11}$) relative to their unnormalized values. A clean first-principles derivation of the $D^{0.93}$ exponent from these combined effects has not yet been achieved. Regardless of mechanism, the empirical depth scaling $J_{\mathrm{topo}}(D) \propto D^{-\alpha_{\mathrm{GELU}}/L}$ with $\alpha_{\mathrm{GELU}} \approx 0.45$ is directly measured and remains valid.
...
\medskip\noindent\textbf{Note on Real Architectures and Future Work:} In synthetic RFF networks, we observe alpha divergence near $J_c \approx 0.35$. However, this critical behavior is not observed in real architectures (ThermoNet, ResNet, VGG) with skip connections and normalization. Skip connections keep $J_{\mathrm{topo}}$ safely above $J_c$, explaining their robust scaling behavior. We present this as a theoretical prediction requiring future empirical validation in real architectures; it should not be treated as an established empirical result.
...
\begin{itemize}
\item \textbf{$D_{\mathrm{eff}}$ width scaling:} Gap 2 measures $D_{\mathrm{eff}} \propto D^{0.93}$, a near-linear scaling that cannot be predicted from standard RMT. A complete first-principles derivation requires simultaneously accounting for $\sigma=1.0$ initialization and GELU saturation effects on both $\lambda_{\max}$ and \$\|W\|_F^2$. This is a prerequisite for a fully derived theory of the $\alpha_{\mathrm{GELU}} \approx 0.45$ exponent.

\item \textbf{$J_c \approx 0.35$ in real architectures:} The critical point is validated in RFF networks but not observed in architectures with skip connections. Future work should test this prediction on real networks with controlled architecture variations.

\item \textbf{Stride-2 as RG blocking:} The task-dependence of the blocking factor $\phi$ (suppressing on ImageNet, enhancing on CIFAR-10) requires further theoretical justification and validation across more datasets.
\end{itemize}

The physical laws provide an effective structural framework (form), but the flesh (constants $B$, $C$, $\gamma_c$) must be determined by thermodynamic contact with the specific environment (dataset).
...
regime originally hypothesized. Gap 2b further shows that standard SGD training from standard initialization cannot access this $D^{0.26}$ regime---within one training step, $\lambda_{\max}$ diverges sharply toward $+\infty$ (numerical overflow at $D \geq 256$), so the network departs from the saturation-like regime entirely rather than converging to it.
...
\item \textbf{RG Derivation:} We derived the complete scaling law including:
    \begin{itemize}
    \item GELU width decay: $\alpha_{\mathrm{GELU}} \approx 0.45$ with empirical depth scaling $J_{\mathrm{topo}}(D) \propto D^{-\alpha_{\mathrm{GELU}}/L}$ ($\alpha_{\mathrm{GELU}} \approx 0.45$) (mechanism is an open question)
