...
\end{equation}
Note: the original hypothesis that this implies a $D^{0.26}$ saturation regime was disproven by Gap 2, which established that standard RMT at initialization gives $\lambda_{\max} \propto D^{0.48}$, not $D^{0.26}$. The physical origin of the empirical depth scaling $J_{\mathrm{topo}}(D) \propto D^{-\alpha_{\mathrm{GELU}}/L}$ may lie in standard RMT eigenvalue concentration combined with finite-width corrections (see Discussion).
