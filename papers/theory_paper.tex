...
\subsection{RMT + GELU: Deriving $\lambda_{\max} \propto D^{0.26}$ and $D_{\mathrm{eff}} \propto D^{0.48}$}
...
\begin{equation}
\boxed{\Lambda_{\max}(J) \;\propto\; D^{0.52}} \qquad\Longrightarrow\qquad \boxed{\lambda_{\max}(J) \;\propto\; D^{0.26}}.
\end{equation}
...
In contrast, width affects condition number \emph{polynomially}. From the RMT+GELU result, $\lambda_{\max}(J_l) \propto D^{0.26}$ and $\|J_l\|_F \propto D^{0.5}$. The condition number per layer is:
\begin{equation}
