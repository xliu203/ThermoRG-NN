...
\item \textbf{RG Derivation of D-Scaling.} We derive the complete D-scaling law from Wilsonian renormalization group theory:
    \begin{itemize}
    \item GELU width decay: $\alpha_{\mathrm{GELU}} \approx 0.45$ explains $J_{\mathrm{topo}}(D) \propto D^{-0.45/L}$
    \item $\beta$ coupling with $\gamma$: $\beta(\gamma) = 0.425\ln(\gamma/\gamma_c) + 0.893$ near $\gamma_c \approx 2.0$
    \item Critical point: $J_c \approx 0.35$ where $\alpha$ diverges over three orders of magnitude
    \end{itemize}
...
\end{cases}
\end{equation}
\textbf{Key property:} GELU saturates to a \emph{constant} (decaying exponentially) for large negative inputs and to \emph{linear} (with slope $1$) for large positive inputs, but with a \emph{smaller effective gain} than the identity.
...
\medskip\noindent\textbf{Derivation of $\alpha_{\mathrm{GELU}}$:} Given $\lambda_{\max} \propto D^{0.52}$ and $\|W\|_F^2 \propto D$, the effective dimension is:
\begin{equation}
...
\item \textbf{$\alpha$ divergence at $J_c \approx 0.35$:} A topological phase transition in the architecture's information flow geometry, analogous to the liquid-gas critical point.
\end{itemize}
...
\item The critical points ($\gamma_c$, $J_c$) are universal constants, not fitted per-system.
...
\item \textbf{RG Derivation:} We derived the complete scaling law including:
    \begin{itemize}
    \item GELU width decay: $\alpha_{\mathrm{GELU}} \approx 0.45$ validated exactly across depths
