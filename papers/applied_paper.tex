...
The two-stage structure exploits the complementary strengths of width and $J_{\mathrm{topo}}$:
...
\item \textbf{$J_{\mathrm{topo}}$ (Optimization):} Within width groups, $J_{\mathrm{topo}}$ controls optimization difficulty. Higher $J_{\mathrm{topo}}$ means more uniform information flow and easier training.
\end{enumerate}

The two channels are statistically independent within groups (partial correlation $r = -0.794$, $p = 0.006$), so optimizing both is additive.
