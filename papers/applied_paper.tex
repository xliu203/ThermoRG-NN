...
\begin{abstract}
Traditional Neural Architecture Search (NAS) is computationally intensive and with limited theoretical foundation. Training hundreds of architectures to find one good one wastes enormous compute. We present ThermoRG-AL, a thermodynamic theory that provides a systematic framework for understanding neural architecture scaling. The key principle is: \emph{fit the law, not the points}. Once the scaling law $L(D) = \alpha D^{-\beta} + E_{\mathrm{floor}}$ is calibrated with $\sim$1 GPU-hour, the entire search space is analytically illuminated. The framework in principle allows optimal architecture to be calculated, not searched. We validate ThermoRG-AL across three experiments: (1) the cooling equation from Phase S1 ($\beta(\gamma) = 0.425\ln(\gamma/2.0) + 0.893$); (2) resolution of Simpson's paradox in Phase B2 (width and $J_{\mathrm{topo}}$ are independent channels); and (3) architecture search where ThermoRG-AL wins Random by 12\% (0.377 vs 0.427). After calibration, prediction is free---fundamentally different from NASWOT/Bayesian optimization which require repeated training evaluations.
\end{abstract}
...
% ==============================================================================
\section{Why NAS Is Expensive: The Computational Cost Problem}
...
\subsection{From Empirical Search to Systematic Framework}

ThermoRG-AL provides a systematic framework for understanding neural architecture scaling. The key is recognizing that the D-scaling law is an equation of state, not an arbitrary fit. Once this form is established by theory, calibration requires only a small number of experiments. The rest is calculation.
...
We presented ThermoRG-AL, a thermodynamic theory of neural architecture search that provides a systematic framework for understanding neural architecture scaling.
...
The physical laws provide an effective structural framework (form). ThermoRG-AL adds the flesh (calibrated constants) to calculate the optimal architecture analytically, not by searching.
