...
\subsection{Step 2: GELU Width Decay ($\alpha_{\mathrm{GELU}} \approx 0.45$)}
...
Given $\lambda_{\max} \propto D^{0.52}$ and $\|W\|_F^2 \propto D$ (each of $D$ output channels has $D \cdot k^2$ weights), the effective dimension is:
\begin{equation}
D_{\mathrm{eff}} = \frac{\|W\|_F^2}{\lambda_{\max}^2} \propto \frac{D}{D^{2 \times 0.52}} = \frac{D}{D^{1.04}} = D^{0.48 \times \gamma_{\mathrm{token}}}.
\end{equation}

The token-mixing correction factor $\gamma_{\mathrm{token}} \approx 0.94$ arises from the structured geometry of convolutional feature maps (spatial correlations reduce the effective degrees of freedom). Empirically, $D_{\mathrm{eff}} \propto D^{0.45}$, giving:
\begin{equation}
...
\subsection{Step 5: Stride‑2 RG Scaling Factor ($\phi = 0.87$)}
