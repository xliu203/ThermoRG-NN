...
We present a phenomenological derivation of the width decay exponent $\alpha_{\mathrm{GELU}}$, combining Random Matrix Theory (RMT) with empirical observations of GELU saturation.
...
Given the empirical spectral scaling $\lambda_{\max} \propto D^{0.52}$ (observed for GELU saturation; see \citep{cohen2021theory,edge-of-stability-2025} for related Edge-of-Stability phenomena) and $\|W\|_F^2 \propto D$ (each of $D$ output channels has $D \cdot k^2$ weights), the effective dimension is:
\begin{equation}
...
\textit{Note on the absolute value:} For the input layer with $D_{\mathrm{data}} = 3072$ (CIFAR-10 pixels) and network width $D \leq 96$, we have $\eta_1 = D_{\mathrm{eff}}^{(1)} / D_{\mathrm{data}} \ll 1$, so $\log \eta_1 < 0$ and $|\log \eta_1| = -\log \eta_1$. The sign of the scaling is rescued by noting that the hidden-layer dynamics (layers $l \geq 2$) dominate the topological correlation; the input-layer dimension mismatch is a boundary effect that does not invalidate the overall $D$-scaling. This subtlety warrants further investigation in future work when applying ThermoRG to higher-dimensional inputs (e.g., high-resolution images or token embeddings).
...
\begin{equation}
\frac{d\beta}{d\gamma} = \frac{a}{\gamma} \quad \Rightarrow \quad \beta(\gamma) = \beta_c + a \cdot \ln\!\Bigl(\frac{\gamma}{\gamma_c}\Bigr).
\end{equation}

\textit{We adopt this as a phenomenological approximation.} While a strict edge-of-stability phase transition near $\gamma_c \approx 2.0$ may exhibit more complex singular behavior \citep{cohen2021theory,edge-of-stability-2025}, the logarithmic form $\beta(\gamma) \propto \ln(\gamma/\gamma_c)$ provides an excellent empirical fit across the observed sub-critical and super-critical regimes.
...
\subsubsection{Empirical Approximation for Algorithm Calibration}

In the empirical implementation of the ThermoRG-AL pre-screening algorithm (Algorithm 1, Step 6), we approximate the topological optimization penalty term $B \cdot J_{\mathrm{topo}}^{\nu}$ using a simplified composite feature $J_{\mathrm{topo}}^{L}$. Recall from the decomposition above that $\nu = \alpha_{\mathrm{GELU}} \cdot L \cdot \theta$. Substituting $J_{\mathrm{topo}}^{L}$ into the linear regression mathematically implies the assumption that the product $k = \alpha_{\mathrm{GELU}} \cdot \theta \approx 1$.

We argue that this is not merely a mathematical convenience, but a physically grounded approximation. Given our empirical measurement of the GELU width-decay exponent $\alpha_{\mathrm{GELU}} \approx 0.45$, the assumption $k \approx 1$ implies an effective optimization penalty exponent of $\theta \approx 1 / 0.45 \approx 2.22$. In classical optimization theory, the convergence rate of first-order methods typically scales with the condition number $\kappa$ (where $\theta = 1$) or $\kappa^{2}$ (where $\theta = 2$ for heavily ill-conditioned problems). In the highly non-convex, pathologically curved loss landscapes of deep neural networks, optimization difficulty is further compounded by variance fluctuations and saddle points, making an effective exponent of $\theta \approx 2.2$ highly consistent with both empirical optimization dynamics and theoretical bounds for deep learning.

Practically, fixing $k \approx 1$ serves as a physically motivated regularization. It allows the algorithm to capture the critical depth-dependent scaling of the topological penalty via the composite feature $J_{\mathrm{topo}}^{L}$ while maintaining a robust single-variable linear regression. This prevents the severe overfitting that would inevitably occur if we attempted to fit $\theta$ as a free parameter on the extremely small calibration dataset ($N_{\mathrm{cal}} = 8$).
...
\end{itemize}
\section*{A.5 Stride-2 as RG Blocking in Residual Networks}

The success of ResNet stems from skip connections enabling training very deep networks. We ask: can we understand the information-flow degradation across residual blocks using RG?

The RG framework motivates a multiplicative decay of the effective information dimension across layers. For a network with $n_s$ stride-2 layers, the RG transformation reduces the resolution by a factor of $2^{n_s}$. Assuming the path integral over intermediate states follows a power-law prior, the log-likelihood of the information dimension after $n_s$ RG steps scales as:
\begin{equation}
\log \mathcal{Z} \sim \beta_0 \cdot 2^{-\Delta \cdot n_s} = \beta_0 \cdot \phi^{\,n_s}, \quad \phi = 2^{-\Delta}.
\end{equation}
where $\beta_0$ is the bare propagation exponent and $\Delta$ is the RG scaling dimension of the relevant operator.

\textit{Note on the scaling form vs. the numerical value of $\Delta$:} The multiplicative form $\beta = \beta_0 \cdot \phi^{\,n_s}$ is theoretically motivated by the RG blocking argument. The precise value of $\Delta$ requires empirical calibration: a first-principles estimate from operator-product expansion gives $\Delta \approx 0.4$ under idealized assumptions, but this is too large for real architectures. Calibrating $\Delta$ using ResNet-18 on CIFAR-10 yields $\Delta_{\mathrm{stride}} \approx 0.20$, giving $\phi = 2^{-0.20} \approx 0.87$. The scaling form itself is the theoretical prediction; the numerical value of $\Delta$ is dataset- and architecture-dependent.

Using the stride-2 scaling model, we can predict the effective information dimension of networks with different numbers of stride-2 layers. With $\Delta$ calibrated from ResNet-18 ($\phi \approx 0.87$), a VGG-style network with no skip connections has $n_s = 5$ and $\beta \approx 0.87^5 \approx 0.50$, which is consistent with the observed ResNet-18 $\beta \approx 0.52$ (where $\beta_0 \approx 0.415$ is separately fitted). This provides a unified scaling framework for architectures with and without explicit down-sampling.
