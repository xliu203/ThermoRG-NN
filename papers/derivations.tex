...
Consider a Conv2d+GELU layer with $D$ input and output channels. At initialization, weights are i.i.d. Gaussian. The GELU activation function~\citep{hendrycks2016gelu}, analyzed through the edge-of-chaos framework pioneered by~\citet{schoenholz2017deep} for tanh/sigmoid/ReLU networks, acts as a \emph{tanh-like} saturating nonlinearity with effective slope $g_{\mathrm{eff}} < 1$.
...
The EOS critical point at $\gamma_c \approx 2.0$ was established by \citet{cohen2021theory}: training dynamics self-regulate such that the sharpness $\lambda_{\max}(H) \approx 2/\eta$. We hypothesize that this corresponds to $\gamma_c \approx 2.0$ via the mapping $\gamma \propto \log(T_{\mathrm{eff}}/T_c)$, analogous to criticality in phase transitions.
...
\textbf{Validation from sub-critical ($\gamma=0.41$) to super-critical ($\gamma=3.39$) regimes}:
\begin{center}
\begin{tabular}{lcccc}
\toprule
...
None (no norm) & 3.39 & 1.117 & 1.117 & 0.000 \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
\item $\gamma_c \approx 2.0$ is the EOS critical value, empirically observed across the normalization types studied
...
\begin{quote}
\textbf{Speculative analysis.} The critical point $J_c \approx 0.35$ is based on $N=5$ RFF network configurations with limited intermediate data (a large gap exists between $J=0.324$ and $J=0.394$). The divergence pattern is suggestive but not independently validated. This is placed in the Appendix as exploratory work, not as a core theoretical result.
\end{quote}
...
\begin{equation}
\mathcal{L}(D) = \alpha \cdot D^{-\beta} + E_{\mathrm{floor}}.
\end{equation}
...
This is the Bayes error rate --- the irreducible minimum error determined by label noise and inherent ambiguity in the data. It is independent of architecture:
\begin{equation}
E_{\mathrm{task}} = \inf_{f} \mathbb{E}_{\mathbf{x}\sim P}[\ell(f(\mathbf{x}), y)] \geq 0.
\end{equation}

\textbf{2. Finite-Width Capacity Correction ($C/D$):}
For finite-width networks, eigenvalue sampling variance causes a $1/D$ correction to the asymptotic error. From random matrix theory, the empirical spectral distribution deviates from the Marchenko-Pastur law by $\mathcal{O}(1/D)$, giving:
\begin{equation}
E_{\mathrm{cap}}(D) = \frac{C}{D},
\end{equation}
where $C > 0$ is a dataset-dependent constant. Larger width $D$ reduces this capacity-limited error.

\textbf{3. Topological Optimization Benefit ($B \cdot J_{\mathrm{topo}}^{\nu}$):}
...
\begin{equation}
\kappa_{\mathrm{tot}} \propto J_{\mathrm{topo}}^{-L}.
\end{equation}

From gradient-based optimization theory, the convergence rate depends on $\kappa_{\mathrm{tot}}$~\citep{boyd2004convex}:
\begin{equation}
\text{convergence rate} \propto \kappa_{\mathrm{tot}}^{-\theta} \propto J_{\mathrm{topo}}^{\,L\theta}.
\end{equation}

The final optimization error scales inversely with the convergence rate:
\begin{equation}
\Delta E_{\mathrm{opt}}(J_{\mathrm{topo}}) = B \cdot J_{\mathrm{topo}}^{\nu}, \quad \nu = L \cdot \theta > 0,
\end{equation}
...
\item $-B \cdot J_{\mathrm{topo}}^{\nu}$: Optimization benefit from uniform information flow (higher $J_{\mathrm{topo}}$ $\Rightarrow$ faster convergence $\Rightarrow$ lower error)
\end{itemize}

This decomposition explains the Simpson's paradox in Phase B2: width and $J_{\mathrm{topo}}$ are separate optimization channels with different scaling behaviors. The $C/D$ term dominates across architectures (explaining why wide networks have low loss), while the $B \cdot J_{\mathrm{topo}}^{\nu}$ term dominates within fixed-width groups (explaining why higher $J_{\mathrm{topo}}$ reduces loss at fixed $D$).

% ==============================================================================
% DERIVATION 5: Stride-2 as RG Blocking: A Phenomenological Scaling Framework
% ==============================================================================

\subsection{A.5 Stride-2 as RG Blocking: A Phenomenological Scaling Framework}\label{sec:stride2-rg}

\begin{quote}
\textit{Speculative analysis.} This section presents a phenomenological RG framework for stride-2 downsampling. The scaling form $\beta = \beta_0 \cdot \phi^{n_s}$ is empirically calibrated from ResNet-18 (N=1 configuration) and should be treated as a forward prediction, not an independently validated result.
\end{quote}

The standard ResNet architecture utilizes skip connections to train deep networks, but downsampling layers (stride-2 convolutions) inevitably disrupt spatial information flow. We propose an RG-inspired phenomenological framework to quantify this degradation.

\subsection*{A.5.1 The Theoretical Functional Form}
A stride-2 convolution reduces spatial resolution by a factor of $b=2$. In a renormalization group (RG) framework, integrating out high-frequency spatial modes rescales the system. This operation applies a multiplicative suppression to the scaling exponent $\beta$, governed by the scaling dimension of the stride perturbation, $\Delta_{\mathrm{stride}}$.

For a network with $n_s$ stride-2 layers, the theoretical RG transformation predicts a multiplicative decay for the effective width-scaling exponent:
\begin{equation}
\beta = \beta_0 \cdot \phi^{n_s}
\end{equation}
where $\phi = 2^{-\Delta_{\mathrm{stride}}}$ is the RG scaling factor, and $\beta_0$ is the bare propagation exponent. This multiplicative functional form is the primary theoretical prediction of our RG framework.

\subsection*{A.5.2 First-Principles Estimation vs. Complex Dynamics}
To predict the exact numerical value of $\phi$, one must determine $\Delta_{\mathrm{stride}}$. Under idealized operator product expansion, assuming the stride operation couples strictly to the gradient squared operator $(\nabla\phi)^2$, a first-principles estimate yields $\Delta_{\mathrm{stride}} \approx 0.4$.

However, this highly idealized assumption overestimates the perturbation penalty in deep neural networks. In reality, the stride perturbation is a complex mixture of operators, mitigated by batch normalization and channel expansion. Therefore, a pure first-principles derivation of $\phi$ is intractable, and $\Delta_{\mathrm{stride}}$ must be treated as an architecture-specific parameter requiring empirical calibration.

\subsection*{A.5.3 Empirical Calibration via ResNet-18}
To utilize the RG scaling form as a predictive tool, we calibrate $\Delta_{\mathrm{stride}}$ empirically using ResNet-18 on CIFAR-10. ResNet-18 contains $n_s = 3$ stride-2 layers. Experimental measurement yields an overall scaling exponent of $\beta \approx 0.28$, with a bare exponent (derived from configurations without striding) of $\beta_0 \approx 0.415$.

By fitting these values to our theoretical form:
\begin{equation}
\phi_{\mathrm{calibrated}} = \left(\frac{0.28}{0.415}\right)^{1/3} \approx 0.87
\end{equation}
This implies an empirical scaling dimension of:
\begin{equation}
\Delta_{\mathrm{stride}} = -\log_2(0.87) \approx 0.20
\end{equation}
This empirically calibrated value ($\Delta_{\mathrm{stride}} \approx 0.20$) is smaller than the idealized estimate ($\approx 0.4$), reflecting the network's ability to partially compensate for information loss through channel expansion.

\subsection*{A.5.4 Forward Prediction}
While $\phi \approx 0.87$ is calibrated from ResNet-18, the power of this phenomenological framework lies in its predictive capability for \emph{other} architectures. By using the calibrated $\phi$, we can predict the effective learning efficiency $\beta$ for architectures with different depths and downsampling strategies (e.g., predicting the scaling behavior of VGG-style plain networks with $n_s=5$ layers, which yields $\beta \approx 0.87^5 \approx 0.50$). This provides a unified scaling framework connecting networks with and without explicit downsampling.
