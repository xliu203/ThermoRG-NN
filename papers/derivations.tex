...
\begin{equation}
D_{\mathrm{eff}} = \frac{\|W\|_F^2}{\lambda_{\max}^2} \propto \frac{D}{D^{2 \times 0.52}} = \frac{D}{D^{1.04}} = D^{0.48 \times \tau_{\mathrm{token}}}.
\end{equation}

The token-mixing correction factor $\tau_{\mathrm{token}} \approx 0.94$ arises from the structured geometry of convolutional feature maps (spatial correlations reduce the effective degrees of freedom). Empirically, $D_{\mathrm{eff}} \propto D^{0.45}$, giving:
\begin{equation}
\boxed{\alpha_{\mathrm{GELU}} = 1 - 0.52 \approx 0.48 \xrightarrow{\tau_{\mathrm{token}}} 0.45}
\end{equation}
