...
Consider a Conv2d+GELU layer with $D$ input and output channels. At initialization, weights are i.i.d. Gaussian. The effective linearization of GELU for signal propagation was analyzed by \citet{danilov2020noise}. The key result is that GELU acts as a \emph{tanh-like} saturating nonlinearity with effective slope $g_{\mathrm{eff}} < 1$.
...
Consider a neural network at a scale‑invariant fixed point, described by an effective field theory with action $S[\phi]$. The RG transformation $\sR_b$ rescales coordinates $\mathbf{x}\to\mathbf{x}/b$ and fields $\phi(\mathbf{x})\to b^{\Delta_\phi}\phi(\mathbf{x}/b)$, where $\Delta_\phi$ is the scaling dimension of the field. Under an infinitesimal perturbation $\delta S$, the change in the scaling dimension of an observable $\sO$ is given by the eigenvalue of the linearized RG flow:

\begin{equation}
\frac{d\Delta_{\sO}}{d\ln b} = \gamma_{\sO} = \langle \sO, \delta S \rangle,
\end{equation}
...
\section*{A.5 Stride-2 as RG Blocking: A Phenomenological Scaling Framework}
