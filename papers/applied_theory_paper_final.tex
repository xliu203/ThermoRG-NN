...
\textbf{Purpose:} Validate the theoretical prediction that stride-2 downsampling acts as an RG blocking operation that suppress the scaling exponent $\beta$.
...
\end{equation}
and the effective expansion ratio becomes $\eta_l^{(\mathrm{stride})} = \eta_l \cdot \zeta_l$. Under RG decimation, the scaling exponent is multiplicatively suppressed:
\begin{equation}
\beta = \beta_0 \cdot (0.87)^{\,n_s},
\end{equation}
where $n_s$ is the number of stride-2 layers and the factor $0.87$ is the scaling dimension of the stride-2 perturbation ($\Delta_{\mathrm{stride}} \approx 0.20$).

\textbf{Method.} Four architectures spanning the ThermoNet family: TN-W8, TN-W16, TN-W32, TN-W64. Two data regimes: $D \in \{5K, 50K\}$ (corresponding to early vs asymptotic scaling). Training: SGD (lr=0.01, momentum=0.9, wd=$5\times10^{-4}$), 50 epochs, 2 seeds (42, 123) per configuration.

\textbf{Expected results.} We predict that max-pooling will show stronger $\beta$ suppression than stride-2 convolution due to its larger effective block size ($\zeta \approx 0.25$ vs $\zeta \approx 0.5$ for typical channel ratios). The scaling exponent suppression formula $\beta = \beta_0 \cdot 0.87^{n_s}$ will be validated across architectures.
