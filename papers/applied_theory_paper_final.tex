...
\subsection{Planned RFF Validation Protocol}
\label{sec:rff-validation}
To test the D-scaling predictions of the theory, we propose a controlled validation protocol using Random Fourier Features (RFF) as a tractable testbed. This allows systematic verification of the Equation of State predictions without the complexity of full neural network training.

\textbf{Teacher-Student Framework.} We construct a teacher network $f^*$ that maps $\mathbb{R}^{d_{\text{input}}}$ to $\mathbb{R}$ via random Fourier features: $f^*(x) = \sum_{k=1}^{K} w_k \phi_k(x)$ where $\phi_k(x) = \cos(2\pi \langle u_k, x \rangle + b_k)$ with $u_k \sim \mathcal{N}(0, I_{d_{\text{input}}})$ and $b_k \sim \text{Uniform}(0, 2\pi)$. The student network has the same RFF structure but with trainable weights.

\textbf{Key Parameters.}
\begin{itemize}
    \item $d_{\text{manifold}}$: Intrinsic dimension of the data manifold (controlled by the correlation structure of inputs)
    \item $d_{\text{task}}$: Dimensionality of the target function (controlled by $K$, the number of RFF components)
    \item $s$: Smoothness exponent of the target function (controlled by the decay of Fourier coefficients)
\end{itemize}

\textbf{Architecture Sweep for $J_{\text{topo}}$.} We vary the student architecture (number of layers $L$, hidden dimension $d_{\text{hidden}}$, spectral efficiency $\eta_l$) and compute the topological Jacobian $J_{\text{topo}} = \sum_l \log \eta_l$ for each configuration. This validates that architectures with $J_{\text{topo}} \approx 0$ (isometric compression) achieve optimal scaling.

\textbf{D-scaling Validation Protocol.}
\begin{enumerate}
    \item Generate synthetic data with controlled $d_{\text{manifold}}$ and $s$ parameters
    \item Train student networks with varying architectures to fit the teacher
    \item Measure scaling exponent $\alpha$ as a function of $d_{\text{manifold}}$ and $N_{\text{params}}$
    \item Verify the prediction $\alpha \propto 2s/d_{\text{manifold}}$ from the Equation of State
    \item Test the $T_{\text{eff}} = T_c$ optimal temperature condition via learning rate scheduling experiments
\end{enumerate}

\textbf{Expected Outcomes.} This controlled setting should reproduce the key predictions: (1) the scaling exponent decreases with increasing $d_{\text{manifold}}$, (2) the optimal architecture satisfies $J_{\text{topo}} \approx 0$, and (3) the optimal training temperature is $T_{\text{eff}}^* = T_c$.

\subsection{Experimental Setup}
\label{sec:experimental-setup}
To validate the proposed unified scaling law, we conduct thermodynamic profiling on a diverse set of architectures and datasets. The validation pipeline follows \cref{alg:first-principles} and evaluates the predicted scaling exponent $\hat{\alpha}$ against the empirically fitted exponent $\alpha$ obtained from actual training runs.
...
\begin{itemize}
    \item \textbf{Predicted scaling exponent $\hat{\alpha}$}: Obtained from thermodynamic profiling (\cref{alg:first-principles}).
