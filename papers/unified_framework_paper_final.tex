...
\bigskip
\noindent\textbf{Extended analysis: $\beta$ and $\alpha$ as functions of $J_{\mathrm{topo}}$.} Contrary to the initial negative report, a corrected statistical analysis reveals strong positive correlations: Pearson $r(\beta, J_{\mathrm{topo}}) = 0.86$ ($p = 0.060$) and $r(\alpha, J_{\mathrm{topo}}^2) = 0.83$ ($p = 0.085$) across the five architectures. The apparent contradiction with the earlier stated results stems from a data-alignment bug in the correlation computation; the raw data clearly show the monotonic trend. With $n=5$ architectures the correlations are marginally significant ($p < 0.10$), suggesting that both $\beta \propto J_{\mathrm{topo}}$ and $\alpha \propto J_{\mathrm{topo}}^2$ are qualitatively supported, though a larger-scale experiment with more architectures would be needed for statistical robustness.
