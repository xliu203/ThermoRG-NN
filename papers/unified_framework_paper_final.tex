...
\begin{equation}
\alpha = \underbrace{k_\alpha}_{\text{scaling constant}} \cdot \underbrace{\Bigl|\log\prod_{l=1}^{L} \eta_l\Bigr|}_{\text{logarithmic compression penalty}} \cdot \underbrace{\frac{2s}{d_{\text{manifold}}}}_{\text{data fundamental limit}} \cdot \underbrace{\psi(T_{\text{eff}})}_{\text{thermodynamic exploration phase}} \cdot \underbrace{\beta(\gamma)}_{\text{cooling dynamics}} \cdot \underbrace{\epsilon(\text{Coupling})}_{\text{coupling correction factor}}.
...
\end{equation}
Here $T_c$ is the \emph{critical temperature} derived from the Edge of Stability condition (see \S\ref{sec:EoS}), and $\\gamma_c \\approx 2.0$ is the EOS critical variance fluctuation. The response function $\psi$ peaks at $T_{\\mathrm{eff}}=T_c$ with $\psi(T_c)=1$, vanishes at $T_{\\mathrm{eff}}\\to0$ (cold deadlock) and $T_{\\mathrm{eff}}\\to\\infty$ (thermal divergence). The cooling dynamics enters through $\\beta(\\gamma) = a\\ln(\\gamma/\\gamma_c) + \\beta_c$, which is not symmetric.
\endRemark}
...
\medskip\noindent\textbf{BN reduces $\beta$ by 15\%.} BatchNorm reduces $\gamma$ from $\gamma_{\\mathrm{None}} = 3.39$ to $\\gamma_{\\mathrm{BN}} = 2.29$, and $\\beta$ from $1.117$ to $0.950$. Using $\\beta(\\gamma) = 0.425\\ln(\\gamma/2.0) + 0.893$:
\begin{equation}
