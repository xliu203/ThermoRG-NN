...
\bigskip
\noindent\textbf{Key negative results (for completeness):} The earlier predictions $\beta \propto J_{\mathrm{topo}}$ (P2) and $\alpha \propto J_{\mathrm{topo}}^2$ (P3) are \emph{not} confirmed. In five architectures $J_{\mathrm{topo}}$ varies by only $\sim 3\times$ (0.12--0.39) while $\beta$ varies by $\sim 3.6\times$ (0.72--2.58), and the correlation is weak ($r = 0.08$). The simple proportionality Ansätze for $\beta$ and $\alpha$ in terms of $J_{\mathrm{topo}}$ do not hold; the theory requires the participation-ratio correction for $\beta$ and a more complex dependency for $\alpha$.
